...
import {Alert,Button,Checkbox,Collapse,Drawer,Modal,Select,Space,Table,Typography} from 'antd';
...
export function DataMergePanel({task,taskId,browse,disabled,onRunning,onRecompare}:{task?:DataTask;taskId:string;browse:boolean;disabled:boolean;onRunning:(active:boolean)=>void;onRecompare:()=>void}) {
 ...
 useEffect(()=>{
  const g=++generation.current;setPlan(undefined);setDeleteConfirmed(false);setRowAction('');setRows([]);setPage(1);setError('');setPlanning(false);planningRef.current=false;pendingModal.current?.destroy();
 ...
 const execute=()=>{
  ...
  pendingModal.current=modal.confirm({title:`确认${mergeModeLabels[mode]}到${direction==='left-to-right'?'右侧':'左侧'}？`,okText:'确认执行',cancelText:'取消',width:640,okButtonProps:{danger:true},content:<Space orientation="vertical"><strong>{target.name} / {target.database}.{target.table}</strong><span>{mergeSummary(plan)}</span><span>范围：{plan.filters.length?plan.filters.map(f=>`${f.field} ${f.op} ${f.value??''}`).join(' AND '):'全表'}</span>{plan.counts.delete>0&&<strong>已确认删除 {plan.counts.delete} 条整行，包含未参与比较的列；范围外记录保留。</strong>}<span>全部符合共享条件的差异；键 {plan.key.join('、')}；字段 {plan.fields.join('、')}</span><Alert type="warning" title="每批独立提交，先前提交不会整体回滚。" description="冲突或失败停止后续批次；停止需等待当前可控边界。结果待核实时不要重试，请先重新比对。"/></Space>,onOk:async()=>{
 ...
 <Space wrap className="merge-toolbar"><Typography.Text strong>数据写入</Typography.Text><Typography.Text type="secondary">{running?'执行中 · 关闭预览不会停止任务':planning?'正在生成计划':plan?mergeSummary(plan):'查看筛选不改变写入范围'}</Typography.Text><Button onClick={()=>setPreviewOpen(true)}>打开写入预览</Button>{running&&<Button disabled={stopping} loading={stopping} onClick={stop}>请求边界停止</Button>}{!running&&record&&<Typography.Text>{syncLabels[record.status]} · 已确认提交 {record.committed} / {record.total}</Typography.Text>}</Space>
 {error&&<Alert type="error" showIcon title={error}/>}
 <Drawer title="数据写入预览" open={previewOpen} onClose={()=>setPreviewOpen(false)} size={880} styles={{wrapper:{maxWidth:'100vw'},body:{overflowWrap:'anywhere'}}}>
 <Space orientation="vertical" size="middle" style={{width:'100%'}}>
 <Typography.Paragraph type="secondary">全部符合当前共享条件的差异；页面筛选、键定位与分页不缩小写入范围。参与字段由上方比对设置确定，匹配键不更新。补齐与合并保留目标独有记录；完全对齐删除范围内目标独有整行，范围外记录保留。</Typography.Paragraph>
