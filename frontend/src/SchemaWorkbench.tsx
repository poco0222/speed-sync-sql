  ...
  return <section className="schema-workbench">
    ...
    <div className="schema-layout">
      <div className="schema-results">
        ...
        <div className="schema-toolbar"><Button icon={<ExportOutlined />} disabled={!comparison || !!busy || disabled} loading={exporting} onClick={() => void exportSummary()}>导出 JSON 摘要</Button></div>
        {busy && <Alert type="info" showIcon title={`${busy}… 已用时 ${Math.max(0, (clock - started) / 1000).toFixed(1)} 秒`} />}
        {notice && <Alert type={notice.type} showIcon title={notice.text} />}
        {!comparison ? <div className="schema-empty"><Empty description={busy ? '正在读取，尚无可用比对结果' : '尚未比对'} /></div> : <>
          ...
          {category === 'ddl' ? <><div className="definition-pair">{definition('left')}{definition('right')}</div></> : <>
