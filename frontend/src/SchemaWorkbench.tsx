  ...
  return <section className="schema-workbench">
    ...
    <div className="schema-layout" style={{ gridTemplateColumns: collapsed ? 'minmax(0, 1fr)' : `${workspace.width ?? 280}px minmax(0, 1fr)` }}>
      {!collapsed && <aside className="schema-navigation" aria-label="库表导航">
        ...
        <label>导航宽度<Slider disabled={disabled} aria-label="导航宽度" min={220} max={360} value={workspace.width ?? 280} onChange={width => setWorkspace(previous => ({ ...previous, width }))} onChangeComplete={width => saveWorkspace({ ...workspace, width })} /></label><Button size="small" disabled={disabled} onClick={() => { const next = { ...workspace, width: 280 }; setWorkspace(next); setCollapsed(false); saveWorkspace(next); }}>重置布局</Button>
      </aside>}
