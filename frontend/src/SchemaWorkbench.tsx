...
export const SchemaWorkbench = forwardRef<SchemaHandle, { state: State; disabled: boolean; onRunning: (running: boolean) => void; onReading?: (running: boolean) => void; onDataRunning: (running: boolean) => void; onDataWriteRunning: (running: boolean) => void; connectionControls?: ReactNode }>(function SchemaWorkbench({ state, disabled, onRunning, onReading, onDataRunning, onDataWriteRunning, connectionControls }, ref) {
  ...
  return <section className="schema-workbench">
    {messageHolder}
    {connectionControls}
    <div className="workbench-connections"><Typography.Text type="secondary" title={`${workspace.left.database} → ${workspace.right.database}`}>来源库：{workspace.left.database || '未选库'} → 目标库：{workspace.right.database || '未选库'}</Typography.Text><Button type="link" onClick={() => setConnectionsOpen(true)}>选择数据库</Button></div>
    <Drawer title="选择数据库" open={connectionsOpen} onClose={() => setConnectionsOpen(false)} size={760}><div className="database-bar">
      {sides.map(side => <section className="database-picker" key={side}>
        ...
        <Button disabled={contextLocked || !state[side] || !!busy} onClick={() => void run('schema', { action: 'databases', side }, `正在读取${sideLabel(side)}数据库`, `${side}-databases`)}>载入库清单</Button>
      </section>)}
    </div></Drawer>
    <div className="workbench-body"><aside className="source-table-nav"><Typography.Text strong>来源表</Typography.Text><Input.Search aria-label="搜索来源表" placeholder="搜索表名" allowClear value={tableSearch} onChange={event => setTableSearch(event.target.value)} /><Button disabled={contextLocked || !state.left || !workspace.left.database || !!busy} onClick={() => void run('schema', {action:'tables',side:'left',database:workspace.left.database}, '正在读取来源表清单', 'left-tables')}>载入 / 刷新表</Button><Tree aria-label="来源表" blockNode disabled={contextLocked || !!busy} selectedKeys={[workspace.left.table]} treeData={sourceTables.map(item => ({key:item.name,title:<span title={item.name}>{item.name}{item.type === 'VIEW' ? '（视图）' : ''}</span>,isLeaf:true}))} onSelect={keys => { const name = String(keys[0] ?? ''); if (name && name !== workspace.left.table) choose('left', 'table', name); }} />{!sourceTables.length && <Typography.Text type="secondary">{lists['left-tables'] ? '未找到表' : '请先载入表清单'}</Typography.Text>}</aside><div className="workbench-content">
    <div className="single-table-heading">
      <div><Typography.Title level={4} title={workspace.left.table}>{workspace.left.table || '选择一张表'}</Typography.Title><Typography.Text type="secondary" title={workspace.right.table}>目标表：{workspace.right.table || '默认与来源同名'}</Typography.Text><Button type="link" disabled={contextLocked || !state.right || !workspace.right.database} onClick={() => { setMappingDraft(workspace.right.table); setMappingOpen(true); }}>修改映射</Button></div>

    </div>
    <Drawer title="目标表映射" open={mappingOpen} onClose={() => setMappingOpen(false)} extra={<Button type="primary" disabled={contextLocked || !mappingDraft.trim()} onClick={() => { if (mappingDraft !== workspace.right.table) choose('right', 'table', mappingDraft); setMappingOpen(false); }}>应用映射</Button>}>
      ...
      <Button disabled={contextLocked || !!busy} onClick={() => void run('schema', { action: 'tables', side: 'right', database: workspace.right.database }, '正在读取目标表清单', 'right-tables')}>载入 / 刷新目标表</Button>
    </Drawer>

    <div className="schema-layout">
      <div className="schema-results">
        <div style={{display:mode === "schema" ? undefined : "none"}}>
        <SyncPanel target={identity('right')} modeControl={modeControl} strategy={strategy} onStrategyChange={changeStrategy} action={<Space><Button disabled={contextLocked || !selected || !!busy} type={comparison ? 'default' : 'primary'} loading={!!busy} onClick={() => void run('compare', { left: workspace.left, right: workspace.right }, '正在读取两端元数据并比对')}>{comparison ? '重新比对' : '开始比对'}</Button>{busy && <Button onClick={() => { invalidate(); setNotice({ type: 'info', text: '已取消读取，旧结果已失效。' }); }}>取消读取</Button>}</Space>} onRecompare={() => void run('compare', { left: workspace.left, right: workspace.right }, '正在重新比对')} comparison={comparison} selected={selectedKeys} disabled={contextLocked || !!busy} onRunning={running => { if (running && !syncWasRunning.current) invalidateData(); syncWasRunning.current = running; onRunning(running); }} onComparison={value => { setComparison(value); setSelectedKeys([]); setInspected(undefined); }} >
        <div className="schema-toolbar"><Typography.Text type="secondary">差异优先 · 查看筛选不改变同步范围</Typography.Text><Button disabled={!comparison || !!busy || disabled} loading={exporting} onClick={() => void exportSummary()}>导出 JSON 摘要</Button></div>
