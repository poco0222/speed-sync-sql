...
export function SyncPanel({comparison,selected,disabled,onRunning,onComparison,onRecompare,action,modeControl,strategy='fill',onStrategyChange,children,target}: {comparison?:Comparison;selected:string[];disabled:boolean;onRunning:(running:boolean)=>void;onComparison:(comparison:Comparison)=>void;onRecompare?:()=>void;action?:ReactNode;modeControl?:ReactNode;strategy?:string;onStrategyChange?:(strategy:string)=>void;children?:ReactNode;target?:string}) {
 ...
 const runningStatus=running && <Alert type="info" showIcon title={stopping?'等待当前语句结束后停止':'正在执行，连接、库表、方向与选区已锁定'} action={<Button disabled={stopping} onClick={()=>{setStopping(true);void request('stop-sync').then(result=>{if(!result.ok)throw new Error(result.error ?? '停止请求失败');}).catch(e=>{setStopping(false);setError(e.message);});}}>请求停止</Button>} />;
 return <section className="sync-panel" aria-label="结构同步">{holder}
  <div hidden={previewOpen}><div className="workbench-actions"><Space wrap>{modeControl}<Segmented aria-label="结构同步方式" value={strategy} disabled={disabled||planning||running} onChange={value=>onStrategyChange?.(value)} options={[{value:'fill',label:'仅新增'},{value:'merge',label:'新增 + 更新'},{value:'align',label:'完全对齐（含删除）'}]} />{action}<Button type="primary" disabled={disabled||!comparison||(!alignAll&&!selected.length)||running} loading={planning} onClick={()=>void build(alignAll)}>预览同步</Button>{(plan||record) && !previewOpen && <Button onClick={()=>setPreviewOpen(true)}>查看同步详情</Button>}</Space></div>
  ...
  <section className="inline-preview" aria-label="结构同步预览" hidden={!previewOpen}><Button onClick={()=>setPreviewOpen(false)}>返回差异</Button>{record&&!running&&onRecompare && <Button disabled={disabled||planning} onClick={()=>{setPreviewOpen(false);onRecompare();}}>重新比对</Button>}
  {runningStatus}
  {error && <Alert type="error" showIcon title={error}/>}
  {plan && <><div className="definition-heading"><strong>目标：{plan.right.name} / {plan.right.database} / {plan.right.table} · {plan.steps.length} 步</strong><Space wrap><Button type="primary" danger disabled={disabled||running||!plan.steps.length} onClick={execute}>执行到目标</Button></Space></div><Typography.Paragraph>来源：{plan.left.name} / {plan.left.database} / {plan.left.table}<br/>范围：{alignAll?'整表对齐（全部增改删）':`选中 ${selected.length} 个对象`}</Typography.Paragraph><SyncOperations plan={plan}/>{!plan.steps.length && <Alert type="info" showIcon title="没有可执行变更"/>}<SyncSteps steps={plan.steps}/><details><summary>SQL 与技术说明</summary><Typography.Paragraph>预览不写入。多条 DDL 不保证整体回滚；停止需等待当前语句结束。</Typography.Paragraph><Space wrap><Button onClick={()=>void exportPlan('copy-sync')}>复制完整 SQL</Button><Button onClick={()=>void exportPlan('export-sync')}>导出 SQL</Button></Space><pre className="sync-sql" tabIndex={0}>{plan.sql}</pre></details></>}
