...
function Application() {
  ...
  const endpoint = (side: 'left' | 'right') => {
    ...
    return <section className="endpoint" aria-label={side === 'left' ? '来源连接' : '目标连接'}>
      ...
      <div className="endpoint-detail">{busy[side] ? <Text>正在测试连接… 已用时 {elapsed(side)} 秒，窗口仍可操作</Text> : connection ? <><span className="mono">{connection.host}:{connection.port}{connection.database ? ` / ${connection.database}` : ''}</span><ResultText result={connection.lastTest} /></> : null}</div>
    </section>;
  };
