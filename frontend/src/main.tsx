...
function Application() {
  ...
  const endpoint = (side: 'left' | 'right') => {
    ...
    return <section className="endpoint" aria-label={side === 'left' ? '来源连接' : '目标连接'}>
      <div className="endpoint-controls"><span className="endpoint-label">{side === 'left' ? '来源' : '目标'}</span><Select aria-label={side === 'left' ? '选择来源连接' : '选择目标连接'} placeholder="选择一个连接" value={state[side] || undefined} allowClear showSearch optionFilterProp="label" disabled={contextLocked || busy[side]} options={state.connections.map(c => ({ value: c.id, label: c.name }))} onChange={id => void select(side, id ?? '')} /><Button icon={<ApiOutlined />} disabled={!connection || contextLocked} loading={busy[side]} onClick={() => void test(side, connection)}>测试连接</Button><Button icon={connection ? <EditOutlined /> : <PlusOutlined />} disabled={contextLocked} onClick={() => openEditor(connection)}>{connection ? '编辑' : '新建'}</Button></div>
      <div className="endpoint-detail">{busy[side] ? <Text>正在测试连接… 已用时 {elapsed(side)} 秒，窗口仍可操作</Text> : connection ? <><span className="mono">{connection.host}:{connection.port}{connection.database ? ` / ${connection.database}` : ''}</span><ResultText result={connection.lastTest} /></> : <Text type="secondary">尚未选择连接</Text>}</div>
    </section>;
  };
