...
function harness(idType='bigint'){
 ...
 vm.runInNewContext(code,{exports:module.exports,require:name=>({'react':hooks,'antd':antd,'./data':data,'./bridge':{request},'./DataMergePanel':{DataMergePanel:'MergePanel'},'react/jsx-runtime':{jsx,jsxs:jsx,Fragment:'Fragment'}}[name]),setTimeout:()=>1,clearTimeout(){}});
 ...
 const nodes=(type)=>{const found=[];const walk=n=>{if(!n||typeof n!=='object')return;if(Array.isArray(n)){n.forEach(walk);return;}if(n.type===type)found.push(n);walk(n.props?.children);walk(n.props?.extra);};walk(tree);return found;};
...
test('draft cancel/no-op preserve result and plan; valid apply invalidates and late detail cannot refill',async()=>{
 const h=harness();h.button('读取键与字段');await h.flush();h.button('开始数据比对');await h.flush();
 ...
 assert.equal(h.calls.filter(c=>c.operation==='data-invalidate').length,initialInvalidations);
 const table=h.nodes('Table').find(n=>n.props.rowKey==='id');table.props.columns.at(-1).render(null,table.props.dataSource[0]).props.onClick();await h.flush();
 ...
 assert.equal(h.nodes('Drawer').find(n=>n.props.title.startsWith('记录详情')).props.open,false);
...
test('draft malformed numeric filter does not invalidate; activity disables settings',async()=>{
 const h=harness();h.button('读取键与字段');await h.flush();h.button('编辑比对条件');await h.flush();h.button('添加 AND 条件');await h.flush();h.button('应用条件');await h.flush();
...
test('closing details preserves status, exact-key location and page without invalidating write scope',async()=>{
 ...
 table.props.columns.at(-1).render(null,table.props.dataSource[0]).props.onClick();await h.flush();h.resolveDetail([]);await h.flush();h.nodes('Drawer').find(n=>n.props.title.startsWith('记录详情')).props.onClose();await h.flush();
...
test('out-of-range drafts preserve scan rows and plan identity; valid boundary still applies',async()=>{
 for(const [type,invalid,valid] of [['tinyint','128','127'],['bigint unsigned','18446744073709551616','18446744073709551615'],['decimal(5,2) unsigned','1000.00','00999.99'],['bit(9)','0200','01ff']]){
  const h=harness(type);h.button('读取键与字段');await h.flush();h.button('开始数据比对');await h.flush();
  await h.flush();
