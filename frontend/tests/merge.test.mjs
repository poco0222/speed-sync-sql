...
test('closing preview preserves polling, write lock and the visible stop action',async()=>{
 ...
 const components=Object.fromEntries(['Alert','Button','Checkbox','Collapse','Drawer','Segmented','Select','Space','Table'].map(name=>[name,name]));
 ...
 const preview=()=>nodes(tree,'div').find(n=>n.props.className==='inline-preview');
 assert.equal(nodes(tree,'Drawer').length,0);
 assert.deepEqual(Array.from(nodes(tree,'Segmented')[0].props.options,v=>v.value),['fill','merge','align']);
 assert.equal(locks.at(-1),true);assert.equal(preview().props.hidden,true);assert.equal(nodes(tree,'RetainedResults')[0],retained);assert.equal(nodes(tree,'ModeControl').length,1);
 nodes(tree,'Button').find(n=>n.props.children==='查看执行结果').props.onClick();tree=render();assert.equal(preview().props.hidden,false);
 nodes(tree,'Button').find(n=>n.props.children==='返回差异').props.onClick();tree=render();assert.equal(preview().props.hidden,true);assert.equal(locks.at(-1),true);assert.equal(nodes(tree,'RetainedResults')[0],retained);assert.equal(nodes(tree,'RetainedResults')[0].props.page,2);
 nodes(tree,'Button').find(n=>n.props.children==='查看执行结果').props.onClick();tree=render();assert.equal(preview().props.hidden,false);
 ...
 nodes(tree,'Button').find(n=>n.props.children==='返回差异').props.onClick();tree=render();
 const toolbar=nodes(tree,'Space').find(n=>n.props.className==='workbench-actions');
